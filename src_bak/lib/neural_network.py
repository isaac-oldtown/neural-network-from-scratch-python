...
class NeuralNetwork:
    def __init__(self, layers_dimensions, input_dim, hidden_activation, loss, output_activation=None):
        ...
        self.nn.extend([self.build_layer(layers_dimensions[layer_dim], layers_dimensions[layer_dim-1]) for layer_dim in range(1, self.n_layers)])
        self.bias=[np.zeros(layer_dim) for layer_dim in layers_dimensions]

        self.hidden_activation=hidden_activation
        self.output_activation=output_activation if output_activation else hidden_activation # typically sigmoid or softmax but not mandatory
        self.loss=loss
    ...
    def get_values(
        ...
        if self.training_cache is None and not include_error:
            return "No predictions, try NeuralNetwork.train() before calling this method."

        if include_error and getattr(self, "error_cache", None) is None:
            return "No error cache, try NeuralNetwork.train() before calling this method."
        ...
        if display:
            ...
            if "values" in result:
                ...
                print(f"  mode   : {'post-activation' if after_activation else 'pre-activation'}")
    ...
    def build_layer(self, layer_dim, neuron_dim, limit=1):
        return np.array([np.random.uniform(-limit, limit, neuron_dim) for _ in range(layer_dim)])
    ...
    def backprop(self, batch_cache, X_batch, Y_batch, learning_rate):
        # dW : parameters error
        # dB : bias error
        dW, dB = [np.zeros_like(W) for W in self.nn], [np.zeros_like(b) for b in self.bias]
